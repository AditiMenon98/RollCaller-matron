...
public class StudentEnrollment extends AppCompatActivity {
    ...
    public void voiceitenroll(View view)
    {
        ...
        Log.v("voiceitthing","before createuser");
        myVoiceIt.createUser(new JsonHttpResponseHandler() {

            @Override
            public void onSuccess(int statusCode, Header[] headers, JSONObject response) {
                Log.v("voiceitthing","bef");


                    Log.v("voiceitthing",response.toString());
                                        //voiceitID = response.getString("userId");


                    Log.d("voiceitthing","JSON Exception");
            ...
            public void onFailure(int statusCode, Header[] headers, Throwable throwable, JSONObject errorResponse) {
                if (errorResponse != null) {
                    Log.v("voiceitthing",errorResponse.toString());
                }
            }
        });

        Log.v("voiceitthing","outsidecreateuser");
        newstudent.setVoiceID(voiceitID);

        Log.v("voiceithing",voiceitID);
